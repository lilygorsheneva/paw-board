...
char decodeAndUpdate(void) {
  ...
  if (!out) {
    ...
    scale_vibration_to_pincount(true, false);
    return 0;
  }
  if (_sent) {
    scale_vibration_to_pincount(true, _post_send_vibrate > _current_time);
    return 0;
  }

  if (!all_pins_stable()) {
    scale_vibration_to_pincount(false, false);
    return 0;
  }
  ...
  // Send key.
    ...
    scale_vibration_to_pincount(false, true);
    return out;
}
